...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_F)
	{
		theWindow->posicioncarro += 0.3;
		theWindow->angulollantas += 5.0;
	}
	if (key == GLFW_KEY_G)
	{
		theWindow->posicioncarro -= 0.3;
	...
	{
		if (theWindow->luzlampara == 1.0f)
		{
			theWindow->luzlampara = 0.0f;
		}
		else
		{
